...
function N3Parser(options) {
  ...
  if (!options.documentURI) {
    ...
    this._baseURI = this._documentURI = options.documentURI;
    this._baseURIRoot = this._documentURIRoot = options.documentURI.replace(documentPart, '');
  }
...
util.inherits(N3Parser, Transform);


// ## Private methods

// ### `_transform` transforms an token stream into a triple stream
N3Parser.prototype._transform = function (token, encoding, done) {
  this._readCallback = this._readCallback(token);
  done();
};

// ### `_flush` finalizes the output stream
N3Parser.prototype._flush = function (callback) {
  if (this._readCallback !== this._readInTopContext)
    this.emit('error', new Error('Unexpected end of file.'));
  callback();
};

// ### `_readInTopContext` reads a token when in the top context.
N3Parser.prototype._readInTopContext = function (token) {
  switch (token.type) {
...
N3Parser.prototype._readSubject = function (token) {
  switch (token.type) {
  case 'explicituri':
...
N3Parser.prototype._readPredicate = function (token) {
  switch (token.type) {
  case 'explicituri':
  case 'abbreviation':
    if (this._baseURI === null || absoluteURI.test(token.value))
...
N3Parser.prototype._readObject = function (token) {
  switch (token.type) {
  case 'explicituri':
    if (this._baseURI === null || absoluteURI.test(token.value))
...
N3Parser.prototype._readBlankNodeHead = function (token) {
...
N3Parser.prototype._readBlankNodeTail = function (token, empty) {
  if (token.type !== 'bracketclose')
    return this._readPunctuation(token);

  // Store blank node triple.
  if (empty !== true)
    this.push({ subject: this._subject,
...
N3Parser.prototype._readDataTypeOrLang = function (token) {
  switch (token.type) {
  case 'type':
    var value;
    if (token.prefix === '') {
...
N3Parser.prototype._readListItem = function (token) {
  ...
  case 'listend':
    ...
    if (stack.length !== 0 && stack[stack.length - 1].type === 'list')
      this.push({ subject: parentTriple.subject,
  ...
  if (prevItemHead === null) {
    ...
    this.push({ subject: prevItemHead,
                predicate: RDF_REST,
                object: itemHead,
                context: 'n3/contexts#default' });
  }
...
N3Parser.prototype._readPunctuation = function (token, empty) {
  var next;
  switch (token.type) {
...
N3Parser.prototype._readPrefix = function (token) {
...
N3Parser.prototype._readPrefixURI = function (token) {
...
N3Parser.prototype._readBaseURI = function (token) {
  if (token.type !== 'explicituri')
...
N3Parser.prototype._readDeclarationPunctuation = function (token) {
...
N3Parser.prototype._getTripleEndReader = function () {
...
N3Parser.prototype._error = function (message, token) {
  this.emit('error', new Error(message + ' at line ' + token.line + '.'));
};
...
N3Parser.prototype.parse = function (n3, callback) {
  // Support legacy callback interface
  if (callback) {
    this.on('error', callback.bind(null));
    this.on('data',  callback.bind(null, null));
    this.on('end',   callback.bind(null, null, null));
  }

  this._lexer.tokenize(n3);
};
